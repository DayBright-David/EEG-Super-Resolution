...
def save_finetune_model(args, epoch, sub, test_block_idx, model, model_without_ddp, optimizer, loss_scaler, model_ema=None, optimizer_disc=None,
               save_ckpt_freq=1):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)
    sub_name = str(sub)
    fold_name = str(test_block_idx)
    ...
    if not getattr(args, 'enable_deepspeed', False):
        ...
        if epoch == 'best':
            ...
            checkpoint_paths.append(output_dir / ("finetune_checkpoint_sub_%s_fold_%s_epoch_%s.pth" % (sub_name, fold_name, epoch_name)))
        ...
        if model_ema is not None:
            client_state['model_ema'] = get_state_dict(model_ema)
        model.save_checkpoint(save_dir=args.output_dir, tag="finetune_checkpoint_sub_%s_fold_%s_epoch_%s.pth" % (sub_name, fold_name, epoch_name), client_state=client_state)
...
def add_gaussian_white_noise(x, target_snr_db=0, target_noise_db=0, mode='noisePower'):
    ...
    if mode == 'targetSNR':
        ...
        mean_noise = 0
        noise_volts = np.random.normal(mean_noise, np.sqrt(target_noise_watts), x_watts.shape)
