...
class SSVEP_TDCA():
    ...
    def predict(self,
        ...
        if self.n_jobs is not None:
            r = Parallel(n_jobs=self.n_jobs)(delayed(partial(_r_tdca_canoncorr_withUV, Y=template_sig, P=test_ref_sig_P, U=U, V=U))(X=a) for a in X_delay)
        else:
            r = []
            for a in X_delay:
                trial_r, trial_spatial_filtered_data = _r_tdca_canoncorr_withUV(X=a, Y=template_sig, P=test_ref_sig_P, U=U, V=U)
                r.append(trial_r)
...
class SSVEP_EAM_TDCA(SSVEP_TDCA):
    def __init__(self,
                 n_component: int = 1,
                 n_jobs: Optional[int] = None,
                 weights_filterbank: Optional[List[float]] = None,
        ...
        super(SSVEP_EAM_TDCA, self).__init__(n_component, n_jobs, weights_filterbank, n_delay)
    ...
    def predict(self,
        ...
        if self.n_jobs is not None:
            r = Parallel(n_jobs=self.n_jobs)(
                delayed(partial(_r_tdca_canoncorr_withUV, Y=template_sig, P=test_ref_sig_P, U=U, V=U))(X=a) for a in
                X_delay)
        else:
            r = []
            for a in X_delay:
                trial_r, trial_spatial_filtered_data = _r_tdca_canoncorr_withUV(X=a, Y=template_sig, P=test_ref_sig_P,
                                                                                U=U, V=U)
                r.append(trial_r)
                spatial_filtered_test_trials.append(trial_spatial_filtered_data)
            # r: list: (epoch, ), array: (filterbank, class_num)
    ...
    def score(self, X, y, test_ref_sig, bad_channel_indices: list=None, returnTemplate=False):
        y_pred, spatial_filtered_test_trials, template_sig, spatial_filtered_template_sig = self.predict(X, test_ref_sig, bad_channel_indices)
        if returnTemplate == False:
            # print('acc: ', accuracy_score(y, y_pred))
            return accuracy_score(y, y_pred), spatial_filtered_test_trials
        else:
...
class SSVEP_CAM_TDCA(SSVEP_TDCA):
    def __init__(self,
                 n_component: int = 1,
                 n_jobs: Optional[int] = None,
                 weights_filterbank: Optional[List[float]] = None,
        ...
        super(SSVEP_CAM_TDCA, self).__init__(n_component, n_jobs, weights_filterbank, n_delay)
        self.tdca_mode = 'CAM'
        self.electrodes_adjacent_matrices = None



    def _CAM_r_tdca_canoncorr_withUV(self,
                                 X: ndarray,
                                 Y: List[ndarray],
                                 P: List[ndarray],
                                 U: ndarray,
    ...
    def predict(self,
        ...
        if self.n_jobs is not None:
            r = Parallel(n_jobs=self.n_jobs)(
                delayed(partial(self._CAM_r_tdca_canoncorr_withUV, Y=template_sig, P=test_ref_sig_P, U=U, V=U, bad_channel_indices=bad_channel_indices))(X=a) for a in
                X_delay)
        else:
            r = []
            for a in X_delay:
                trial_r, trial_spatial_filtered_data = self._CAM_r_tdca_canoncorr_withUV(X=a, Y=template_sig, P=test_ref_sig_P,
    ...
    def score(self, X, y, test_ref_sig, bad_channel_indices: list=None, returnTemplate=False):
        y_pred, spatial_filtered_test_trials, template_sig, spatial_filtered_template_sig = self.predict(X,
...
# class robust_weight_net(nn.Module):
#     def __init__(self, hidden_size=80):
#         super(robust_weight_net, self).__init__()
#         self.weight_net = nn.Sequential(nn.Linear(80, hidden_size),
#                                         nn.ReLU(),
...
#                                         )
#         # self.weight_net = nn.Sequential(nn.Linear(40*3, 40),
#         #                                 )
#
#     def forward(self, x):
#         x = self.weight_net(x)
...
class SSVEP_ROBUST_TDCA():
    def __init__(self,
        ...
        self.method = method
        self.normal_tdca = SSVEP_TDCA(n_component, n_jobs, weights_filterbank, n_delay)
        self.eam_tdca = SSVEP_EAM_TDCA(n_component, n_jobs, weights_filterbank, n_delay,
                               electrodes_names=ssvep_selected_channels, interpolation_matrices=interpolation_matrices)
        self.cam_tdca = SSVEP_CAM_TDCA(n_component, n_jobs, weights_filterbank, n_delay)
        self.tdca_mode = 'ROBUST'
        if self.method == 'weights' or self.method == 'all':
            self.device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
            # self.checkpoints = []
            # if weight_path is None:
            #     raise ValueError('weight path must be assigned when trainWeight is True!')
            # # 保证读进来的文件顺序符合从小到大
            # checkpoint_files_num = len(os.listdir(weight_path))
            # checkpoint_files = ['fold_' + str(fold_idx) + '.pth' for fold_idx in range(1, checkpoint_files_num+1)]
            #
            # checkpoint_paths = [weight_path + '/' + checkpoint_file for checkpoint_file in checkpoint_files]
            # self.checkpoints = [torch.load(checkpoint_path) for checkpoint_path in checkpoint_paths]
            # # load checkpoint
            # self.weight_net_list = [robust_weight_net().to(self.device) for fold_idx in range(1, checkpoint_files_num+1)]
            # for fold_idx, net in enumerate(self.weight_net_list):
            #     net.load_state_dict(self.checkpoints[fold_idx])
    ...
    def fit(self,
        ...
        self.normal_tdca.fit(X, Y, train_ref_sig)
        self.eam_tdca.fit(X, Y, train_ref_sig)
        self.cam_tdca.fit(X, Y, train_ref_sig)

        if self.method == 'weights' or self.method == 'all':
            self.moe = weighted_sum().to(self.device)

            data_save_root = '/home/daibo/classification_TDCA_TRF/dataset/Exp_pretrain_channel_attacker/weightnet_data/' + sub_name
            if not os.path.isdir(data_save_root):
                os.makedirs(data_save_root)
                print('create dirs: ', data_save_root)
            print('******* create data *******')
            train_correlations, train_labels = self.create_channel_attacked_correlations(X, Y, train_ref_sig)
            np.save(data_save_root + '/train_correlations_fold_' + str(fold_idx+1) + '.npy', train_correlations)
            np.save(data_save_root + '/train_labels_fold_' + str(fold_idx+1) + '.npy', train_labels)
            # train_correlations = np.load(data_save_root + '/train_correlations_fold_' + str(fold_idx + 1) + '.npy')
            # train_labels = np.load(data_save_root + '/train_labels_fold_' + str(fold_idx + 1) + '.npy')
            # 只使用EAM和CAM的结果
    ...
    def create_channel_attacked_correlations(self, X, Y, ref_sig):
        ...
        for ch_idx in range(X[0].shape[1] + 1):
            ...
            self.normal_tdca.predict(X_attack, ref_sig)


            if ch_idx < X[0].shape[1]:
                self.eam_tdca.predict(X_attack, ref_sig, [ch_idx])
                self.cam_tdca.predict(X_attack, ref_sig, [ch_idx])
            else:
                self.eam_tdca.predict(X_attack, ref_sig, [])
                self.cam_tdca.predict(X_attack, ref_sig, [])

            normal_correlations = self.normal_tdca.test_trials_correlations
            eam_correlations = self.eam_tdca.test_trials_correlations
            cam_correlations = self.cam_tdca.test_trials_correlations
    ...
    def score(self, X, y, test_ref_sig, bad_channel_indices: list=None):
        ...
        normal_acc, _ = self.normal_tdca.score(X, y, test_ref_sig)
        eam_acc, _ = self.eam_tdca.score(X, y, test_ref_sig, bad_channel_indices)
        cam_acc, _ = self.cam_tdca.score(X, y, test_ref_sig, bad_channel_indices)
...
#                  n_delay: int = 0,
#                  electrodes_adjacent_matrix: ndarray = None,
#                  method: str = None,  # max, sum, weights
#                  ):
#
#         if method is None:
#             raise ValueError('Method must not be None! Must be one of max, sum or weights')
#         self.method = method
#         self.normal_tdca = SSVEP_TDCA(n_component, n_jobs, weights_filterbank, n_delay)
#         self.eam_tdca = SSVEP_EAM_TDCA(n_component, n_jobs, weights_filterbank, n_delay, electrodes_adjacent_matrix)
#         self.cam_tdca = SSVEP_CAM_TDCA(n_component, n_jobs, weights_filterbank, n_delay)
#         self.tdca_mode = 'ROBUST'
#         if self.method == 'weights':
#             self.device = 'cuda:0' if torch.cuda.is_available() else 'cpu'
...
#             X: Optional[List[ndarray]] = None,
#             Y: Optional[List[int]] = None,
#             X_test: Optional[List[ndarray]] = None,
...
#             self.moe = weighted_sum().to(self.device)
#
#             data_save_root = '/home/daibo/classification_TDCA_TRF/dataset/Benchmark/weightnet_data/' + sub_name
#             if not os.path.isdir(data_save_root):
#                 os.makedirs(data_save_root)
#                 print('create dirs: ', data_save_root)
#             # print('******* create data *******')
#             train_correlations, train_labels = self.create_channel_attacked_correlations(X_test, Y_test, train_ref_sig)
#             # np.save(data_save_root + '/train_correlations_fold_' + str(fold_idx+1) + '.npy', train_correlations)
#             # np.save(data_save_root + '/train_labels_fold_' + str(fold_idx+1) + '.npy', train_labels)
#             # train_correlations = np.load(data_save_root + '/train_correlations_fold_' + str(fold_idx + 1) + '.npy')
#             # train_labels = np.load(data_save_root + '/train_labels_fold_' + str(fold_idx + 1) + '.npy')
...
#                 X_attack.append(attacked_trial)
#
#             acc, _ = self.normal_tdca.score(X_attack, Y, ref_sig)
#             fold_train_acc[ch_idx, 0] = acc
#
#             if ch_idx < X[0].shape[1]:
#                 acc, _ = self.eam_tdca.score(X_attack, Y, ref_sig, [ch_idx])
#                 fold_train_acc[ch_idx, 1] = acc
#                 acc, _ = self.cam_tdca.score(X_attack, Y, ref_sig, [ch_idx])
#                 fold_train_acc[ch_idx, 2] = acc
#             else:
#                 acc, _ = self.eam_tdca.score(X_attack, Y, ref_sig, [])
#                 fold_train_acc[ch_idx, 1] = acc
#                 acc, _ = self.cam_tdca.score(X_attack, Y, ref_sig, [])
#                 fold_train_acc[ch_idx, 2] = acc
#
#             normal_correlations = self.normal_tdca.test_trials_correlations
#             eam_correlations = self.eam_tdca.test_trials_correlations
...
#         labels = np.concatenate(labels, axis=0)
#
#         print('fold train_acc: ', fold_train_acc)
#
#         return correlations, labels
#
#
...
#         elif self.method == 'max':
#             robust_correlations = []
#             for normal_correlation, eam_correlation, cam_correlation in zip(normal_correlations, eam_correlations, cam_correlations):
#                 robust_correlations.append(np.max(np.concatenate((normal_correlation, eam_correlation, cam_correlation), axis=0), axis=0))
#
#             robust_Y_pred = [int(np.argmax(trial_correlations)) for trial_correlations in robust_correlations]
#             robust_acc = accuracy_score(y, robust_Y_pred)
#         elif self.method == 'sum':
#             robust_correlations = []
#             for idx, (normal_correlation, eam_correlation, cam_correlation) in enumerate(zip(normal_correlations, eam_correlations,
#                                                                             cam_correlations)):
#
#                 # print('--------------', str(idx), '--------------')
#                 # print(normal_correlation)
#                 # print(np.argmax(normal_correlation))
#                 # print(eam_correlation)
#                 # print(np.argmax(eam_correlation))
#                 # print(cam_correlation)
#                 # print(np.argmax(cam_correlation))
#
#                 normal_correlation = scs.softmax(normal_correlation)
#                 eam_correlation = scs.softmax(eam_correlation)
#                 cam_correlation = scs.softmax(cam_correlation)
...
#                 # print(cam_correlation)
#                 weighted_sum = normal_correlation + eam_correlation + cam_correlation
#                 # print(np.argmax(weighted_sum))
#                 # normal_correlation = np.abs(normal_correlation)
#                 # eam_correlation = np.abs(eam_correlation)
#                 # cam_correlation = np.abs(cam_correlation)
#                 #
#                 # weighted_sum = normal_correlation / np.sum(normal_correlation) \
#                 #                         + eam_correlation / np.sum(eam_correlation) \
#                 #                         + cam_correlation / np.sum(cam_correlation)
#                 robust_correlations.append(weighted_sum)
#
#             robust_Y_pred = [int(np.argmax(trial_correlations)) for trial_correlations in robust_correlations]
#             # print(robust_Y_pred)
