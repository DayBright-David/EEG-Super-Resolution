...
from dataset import build_Benchmark_dataset_for_finetune
...
def get_args():
    ...
    parser.add_argument('--drop_rate', type=float, default=0.1,
                        help='Drop rate for MLP layer')
    parser.add_argument('--attn_drop', type=float, default=0.1,
                        help='Drop rate for attention layer')

    parser.add_argument('--checkpoint_path',
                        default='./checkpoints/labram_base', type=str,
    ...
    parser.add_argument('--output_dir', default='./finetune_checkpoints',
    ...
    parser.add_argument('--sub', default=1, type=int,
                        help='sub idx (start from 1)')
    parser.add_argument('--test_block_idx', default=0, type=int,
                        help='block for test')
...
def main(args):
    ...
    checkpoint = torch.load(args.checkpoint_path + '/checkpoint-' + str(args.checkpoint_epoch) + '.pth',
                            map_location='cpu')
    ...
    dataset_train, dataset_test = build_Benchmark_dataset_for_finetune(root_dir=args.dataset_root_dir, winLEN=args.input_size/args.srate, sub=args.sub, test_block_idx=args.test_block_idx)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, data_loader_train_list,
            optimizer, args.patch_size, device, epoch, loss_scaler,
        ...
        if args.output_dir:
            utils.save_finetune_model(sub=args.sub, test_block_idx=args.test_block_idx,
